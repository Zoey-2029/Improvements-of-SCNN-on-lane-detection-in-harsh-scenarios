...
def train_net(dataset_dir, weights_path=None, net_flag='vgg'):
    ...
    #optimizer = tf.train.AdamOptimizer(learning_rate=learning_rate / 5)
    # optimizer = tf.train.MomentumOptimizer(learning_rate=learning_rate / 3, momentum=0.5, use_nesterov=True)
    ...
    # model_save_dir = 'model/culane_lanenet/culane_scnn_background_coef_0.2'
    # model_save_dir = 'model/culane_lanenet/culane_scnn_vgg_restore'
    # model_save_dir = 'model/culane_lanenet/culane_scnn_vgg_SGD_Nesterov'
    # model_save_dir = 'model/culane_lanenet/culane_scnn_resnet18_bg0.2'
    # model_save_dir = 'model/culane_lanenet/culane_scnn_resnet' # pretrain resnet
    model_save_dir = 'model/culane_lanenet/culane_scnn_resnet_restore' # restore from pretrained resnet
    # model_save_dir = 'model/culane_lanenet/culane_scnn_vgg_shuffle'
    # model_save_dir = 'model/culane_lanenet/culane_scnn_data_augmented'
    ...
    with tf.Session(config=sess_config) as sess:
        with sess.as_default():
            print('---here', net_flag == 'resnet_restore', weights_path is not None)
            if net_flag == 'backbone_restore' and weights_path is not None:
                # Added method to load the vgg/resnet backbone variables from checkpoint file
                # Note: This is for restoring models pretrained on CULane without the message passing layers
                # weight_path should look like "model/culane_lanenet/culane_scnn_vgg/culane_lanenet_vgg_2021-03-10-01-30-06.ckpt-1000"
                init = tf.global_variables_initializer()
                # print("global:", [v for v in tf.trainable_variables()])
                # print("ckpt:", [v for v in tf.train.list_variables(weights_path)])
                reader = tf.train.load_checkpoint(weights_path)
                var_map = reader.get_variable_to_shape_map()
                variables_to_restore = [v for v in tf.trainable_variables() if v.name.split(':')[0] in var_map]
                
                # print("found variables:", len(variables_to_restore), variables_to_restore)
                saver_list = tf.train.Saver(var_list=variables_to_restore)
                sess.run(init)
                saver_list.restore(sess, weights_path)

            if net_flag == 'resnet_restore' and weights_path is not None:
                # Added method to load the resnet backbone variables from checkpoint file
                # resnet weights_path = resnet-18-tensorflow/init/model.ckpt
                print('Train from resnet restore')
                init = tf.global_variables_initializer()
                sess.run(init)
                reader = tf.train.load_checkpoint(weights_path)
                # new_map = tf.train.list_variables(weights_path)
                # print(new_map)
                var_map = reader.get_variable_to_shape_map()
                # print("global:", [v for v in tf.trainable_variables()])
                # print("ckpt:", [v for v in tf.train.list_variables(weights_path)])
                ...
                for k in var_map.keys():
                    ...
                    if len(kk)>=3:
                        if kk[-1] == 'kernel':
                            kk[-1] = 'W'
                        newk = kk[0] + '/' + kk[1] + '/' + kk[2]
                        var_map[newk] = var_map.pop(k)
                
                for v in tf.trainable_variables():
                    vv = v.name.split('/')[-3:]
                    vv = vv[0] + '/' + vv[1] + '/' + vv[2].split(':')[0]
                    if vv in var_map:
                        if (vv[-1] == 'W'):
                            ww = vv[:-1] + 'kernel'
                            value = reader.get_tensor(ww)
                        else:
                            value = reader.get_tensor(vv)
                        # print("\tvalue=", value)
                        _op = tf.assign(v, value)
                        sess.run(_op)
                        # variables_to_restore.append(v)
                # print("found variables:", len(variables_to_restore), variables_to_restore)


            if net_flag != 'resnet' and weights_path is None:
                log.info('Training from scratch')
                init = tf.global_variables_initializer()
                sess.run(init)
            if net_flag == 'model_restore' and weights_path is not None:
        ...
        for epoch in range(CFG.TRAIN.EPOCHS):
            ...
            epoch_accuracy_dict[epoch] = train_accuracy
            ...
            if epoch % 1000 == 0:
                ...
                saver_full.save(sess=sess, save_path=model_save_path, global_step=epoch)
        ...
        convergence_file = open("./convergence_trend/convergence_trend.txt", "w+")
        convergence_file.write(str(epoch_accuracy_dict))
