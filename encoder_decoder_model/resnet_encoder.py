...
class ResNetEncoder(cnn_basenet.CNNBaseModel):
    ...
    def _resblock(self, input_tensor, out_dims, _is_training, use_bias=True, downsample=False, scope='resblock') :
        with tf.variable_scope(scope):
            x = self.layerbn(inputdata=input_tensor, is_training=_is_training, name='bn_0')
            x = self.relu(inputdata=x, name='relu')

            if downsample :
                x = self.conv2d(inputdata=x, out_channel=out_dims,
                               kernel_size=3, stride=2,
                               use_bias=use_bias, name='conv_0')

                input_tensor = self.conv2d(inputdata=input_tensor, out_channel=out_dims,
                               kernel_size=1, stride=2,
                               use_bias=use_bias, name='conv_init')
            else:
                x = self.conv2d(inputdata=x, out_channel=out_dims,
                               kernel_size=3, stride=1,
                               use_bias=use_bias, name='conv_0')
            x = self.layerbn(inputdata=x, is_training=_is_training, name='bn_1')
            x = self.relu(inputdata=x, name='relu')
            x = self.conv2d(inputdata=x, out_channel=out_dims,
                               kernel_size=3, stride=1,
                               use_bias=use_bias, name='conv_1')
        return x + input_tensor


    def bottle_resblock(self, input_tensor, out_dims, _is_training, use_bias=True, downsample=False, scope='bottle_resblock') :
        with tf.variable_scope(scope):
            x = self.layerbn(inputdata=input_tensor, is_training=_is_training, name='bn_1x1_front')
            shortcut = self.relu(inputdata=x, name='relu')

            x = self.conv2d(inputdata=shortcut, out_channel=out_dims,
                               kernel_size=1, stride=1,
                               use_bias=use_bias, name='conv_1x1_front')
            x = self.layerbn(inputdata=x, is_training=_is_training, name='bn_3x3')
            x = self.relu(inputdata=x, name='relu')
            ...
            if downsample :
                x = self.conv2d(inputdata=x, out_channel=out_dims,
                               kernel_size=3, stride=2,
                               use_bias=use_bias, name='conv_0')
                shortcut = self.conv2d(inputdata=shortcut, out_channel=out_dims*4,
                               kernel_size=1, stride=2,
                               use_bias=use_bias, name='conv_init')
            else :
                x = self.conv2d(inputdata=x, out_channel=out_dims,
                ...
                shortcut = self.conv2d(inputdata=shortcut, out_channel=out_dims*4,
                               kernel_size=1, stride=1,
                               use_bias=use_bias, name='conv_init')
            x = self.layerbn(inputdata=x, is_training=_is_training, name='bn_1x1_back')
            x = self.relu(inputdata=x, name='relu')
            x = self.conv2d(inputdata=x, out_channel=out_dims*4,
                               kernel_size=1, stride=1,
                               use_bias=use_bias, name='conv_1x1_back')
        return x + shortcut
    ...
    def encode(self, input_tensor, name):
        ...
        with tf.variable_scope(name):
            if self.res_n < 50 :
                residual_block = self._resblock
            else :
                residual_block = self.bottle_resblock

            residual_list = self.get_residual_layer(self.res_n)

            channels = 64
            x = self.conv2d(inputdata=input_tensor, out_channel=channels,
                               kernel_size=3, stride=1, name='conv')
            print('1range', x.get_shape().as_list()[1])
            for i in range(residual_list[0]):
                x = residual_block(x, out_dims=channels, _is_training=self._is_training, downsample=False, scope='resblock0_' + str(i))

            print('2range', x.get_shape().as_list()[1])
            x = residual_block(x, out_dims=channels*2, _is_training=self._is_training, downsample=True, scope='resblock1_0')
            for i in range(1, residual_list[1]) :
                x = residual_block(x, out_dims=channels*2, _is_training=self._is_training, downsample=False, scope='resblock1_' + str(i))
            
            print('3range', x.get_shape().as_list()[1])
            x = residual_block(x, out_dims=channels*4, _is_training=self._is_training, downsample=True, scope='resblock2_0')
            for i in range(1, residual_list[2]) :
                x = residual_block(x, out_dims=channels*4, _is_training=self._is_training, downsample=False, scope='resblock2_' + str(i))
            
            print('4range', x.get_shape().as_list()[1])
            x = residual_block(x, out_dims=channels*8, _is_training=self._is_training, downsample=True, scope='resblock3_0')
            for i in range(1, residual_list[3]) :
                x = residual_block(x, out_dims=channels*8, _is_training=self._is_training, downsample=False, scope='resblock3_' + str(i))
            print('5range', x.get_shape().as_list()[1])
            # # conv stage 1_1
            ...
            with tf.variable_scope("convs_6_1"):
                ...
                print('---71.1', feature_list_old[0].shape, feature_list_old[1].shape)

            for cnt in range(2, conv_5_5.get_shape().as_list()[1]):
                print('range', conv_5_5.get_shape().as_list()[1])
                with tf.variable_scope("convs_6_1", reuse=True):
                    ...
                    feature_list_new.append(conv_6_1)
                    print('71.2---', feature_list_new[cnt - 1].shape, feature_list_old[cnt].shape)
