...
class ResNetEncoder(cnn_basenet.CNNBaseModel):
    ...
    def encode(self, input_tensor, name):
        ...
        with tf.variable_scope(name):
            ...
            feature_list_old = []
            feature_list_new = []
            for cnt in range(conv_6_5.get_shape().as_list()[1]):
                feature_list_old.append(tf.expand_dims(conv_6_5[:, cnt, :, :], axis=1))
            feature_list_new.append(tf.expand_dims(conv_6_5[:, 0, :, :], axis=1))
            ...
            with tf.variable_scope("convs_7_1"):
                conv_7_1 = tf.add(tf.nn.relu(tf.nn.conv2d(feature_list_old[0], w1, [1, 1, 1, 1], 'SAME')),
                                  feature_list_old[1])
                print('---71.1', feature_list_old[0].shape, feature_list_old[1].shape)
                feature_list_new.append(conv_7_1)
            
            for cnt in range(2, conv_6_5.get_shape().as_list()[1]):
                print('65range', conv_6_5.get_shape().as_list()[1])
                with tf.variable_scope("convs_7_1", reuse=True):
                    conv_7_1 = tf.add(tf.nn.relu(tf.nn.conv2d(feature_list_new[cnt - 1], w1, [1, 1, 1, 1], 'SAME')),
            ...
            # down to top #
            print('-------',len(feature_list_new), int(CFG.TRAIN.IMG_HEIGHT / 8) - 1)
            feature_list_old = feature_list_new
            feature_list_new = []
            length = int(CFG.TRAIN.IMG_HEIGHT / 32) - 1  # 8
            ...
            w2 = tf.get_variable('W2', [1, 9, 128, 128], initializer=tf.contrib.layers.xavier_initializer())
                                 # initializer=tf.random_normal_initializer(0, math.sqrt(2.0 / (9 * 128 * 128 * 5))))
            with tf.variable_scope("convs_7_2"):
                conv_7_2 = tf.add(tf.nn.relu(tf.nn.conv2d(feature_list_old[length], w2, [1, 1, 1, 1], 'SAME')),
                                  feature_list_old[length - 1])
                feature_list_new.append(conv_7_2)

            for cnt in range(2, conv_6_5.get_shape().as_list()[1]):
                with tf.variable_scope("convs_7_2", reuse=True):
                    conv_7_2 = tf.add(tf.nn.relu(tf.nn.conv2d(feature_list_new[cnt - 1], w2, [1, 1, 1, 1], 'SAME')),
            ...
            w3 = tf.get_variable('W3', [9, 1, 128, 128], initializer=tf.contrib.layers.xavier_initializer())
                                 # initializer=tf.random_normal_initializer(0, math.sqrt(2.0 / (9 * 128 * 128 * 5))))
            with tf.variable_scope("convs_7_3"):
                conv_7_3 = tf.add(tf.nn.relu(tf.nn.conv2d(feature_list_old[0], w3, [1, 1, 1, 1], 'SAME')),
                                  feature_list_old[1])
                feature_list_new.append(conv_7_3)

            for cnt in range(2, processed_feature.get_shape().as_list()[2]):
                with tf.variable_scope("convs_7_3", reuse=True):
                    conv_7_3 = tf.add(tf.nn.relu(tf.nn.conv2d(feature_list_new[cnt - 1], w3, [1, 1, 1, 1], 'SAME')),
                                      feature_list_old[cnt])
                    feature_list_new.append(conv_7_3)
            ...
            feature_list_old = feature_list_new
            feature_list_new = []
            length = int(CFG.TRAIN.IMG_WIDTH / 32) - 1  # 8
            feature_list_new.append(feature_list_old[length])

            w4 = tf.get_variable('W4', [9, 1, 128, 128], initializer=tf.contrib.layers.xavier_initializer())
                                 # initializer=tf.random_normal_initializer(0, math.sqrt(2.0 / (9 * 128 * 128 * 5))))
            with tf.variable_scope("convs_7_4"):
                conv_7_4 = tf.add(tf.nn.relu(tf.nn.conv2d(feature_list_old[length], w4, [1, 1, 1, 1], 'SAME')),
                                  feature_list_old[length - 1])
                feature_list_new.append(conv_7_4)

            for cnt in range(2, processed_feature.get_shape().as_list()[2]):
                with tf.variable_scope("convs_7_4", reuse=True):
                    conv_7_4 = tf.add(tf.nn.relu(tf.nn.conv2d(feature_list_new[cnt - 1], w4, [1, 1, 1, 1], 'SAME')),
                                      feature_list_old[length - cnt])
                    feature_list_new.append(conv_7_4)
